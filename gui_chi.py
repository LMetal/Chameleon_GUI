...
def init_dati_chi(root, progressbar, loading_label, loaded_label, load_error_label, init_combo_callback):
    global server_dict, nodes, images_dict, keypairs, secgroups, networks
    ...
    try:
        ...
        with ThreadPoolExecutor() as executor:
            futures = {
                ...
                'types': executor.submit(utils.get_nodes),
...
def create_chi_tab(notebook, password):
    global utils
    utils = OpenStackManager("app-cred-CHI@TACC.sh")
    ...
    combo_date = ttk.Combobox(datetime_frame, state='readonly', values=date_options, width=12)
    combo_date.grid(row=0, column=0, padx=(0, 5))
    combo_date.set(date_options[0])
    combo_time = ttk.Combobox(datetime_frame, state='readonly', values=time_options, width=8)
    combo_time.grid(row=0, column=1)
